...
## Lec13 - 흐림 효과
* 원리
  * 원본 이미지와 필터 간의 컨볼루션을 통해 이미지의 픽셀 값을 재 조정하여 흐리게 만든다.

* 사용 코드
  * 단순 흐림 효과 함수 :: cv2.blur(src, ksize, anchor, borderType)

* 커널
  * 이미지에서 (x, y)의 픽셀과 해당 픽셀 주변을 포함한 작은 크기의 공간을 의미
  * 이 영역 각각의 특정한 수식이나 함수 등을 적용해 새로운 이미지를 얻는 알고리즘에서 사용

* 고정점
  * 커널을 통해 컨벌루션된 값을 할당할 지점
  * Default = (-1, -1) 커널의 중심점
  
* 이미지 테두리 처리법(마스크 크기 / 2 만큼 입력 영상의 테두리 영역)
  * 모두 0으로 채운다
  * 입력 영상의 테두리 영역의 값을 복사한다
  * 컨볼루션 유효 위치의 값을 넣어준다.
